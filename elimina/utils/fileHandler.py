...
from elimina.utils.utils import COLORS, SCOPES
...
def data_update(guildID):
    ...
    request = (
        SERVICESHEET.spreadsheets()
        .values()
        .clear(spreadsheetId=SHEETID, range=range, body=clear_values_request_body)
    )
    request.execute()
        ...
        spreadsheet_data = [
            {
                "deleteDimension": {
                    "range": {
                        "sheetId": GID - ID,
                        "dimension": "ROWS",
                        "startIndex": int(dataidx) - 1,
                        "endIndex": int(dataidx),
                    }
                }
            }
        ]
        update_data = {"requests": spreadsheet_data}
        request = SERVICESHEET.spreadsheets().batchUpdate(
            spreadsheetId=SHEETID, body=update_data
        )
        request.execute()
