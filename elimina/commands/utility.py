...
from elimina.utils.fileHandler import DATA
...
class Utility(commands.Cog):
    ...
    async def snipe(self, ctx: commands.Context) -> None:
        ...
        if (
            ...
            and not ctx.author.guild_permissions.administrator
            and str(DATA[str(ctx.guild.id)][1]) == "0"
        ):
            errorEmbed = Embed(
                title=None,
                color=COLORS["red"],
        ...
        embed = discord.Embed(
            description=str(snipe["content"]), colour=COLORS["accent"]
        )
        embed.set_footer(
    ...
    async def editsnipe(self, ctx: commands.Context) -> None:
        ...
        embed = discord.Embed(
            description=str(edit_snipe["content"]), colour=COLORS["accent"]
        )
        embed.set_footer(
            text=f"sniped by {ctx.author.name}#{ctx.author.discriminator}",
            icon_url=ctx.author.avatar_url,
        ...
        embed.set_author(
            ...
            icon_url=edit_snipe["author"].avatar_url,
        )
        await ctx.send(embed=embed)
