...
class Info(commands.Cog):
    ...
    async def _help(self, ctx: commands.Context) -> None:
        embedHelp = Embed(title="**Commands**", colour=COLORS["accent"])
        ...
        embedHelp.add_field(
            name="`~imgsnipe` : __(Admin only)__",
        ...
        embedHelp.add_field(
            name="Support Server", value="*https://discord.gg/mXPGpTMsqS*", inline=False
        )
        embedHelp.add_field(
            name="_Messages from Elimina are deleted after 1 minute in toggled on channels_",
            value="*For help contact: eliminabot@gmail.com*",
            inline=False,
        )
        embedHelp.set_author(
            name="Elimina Bot",
            url="https://github.com/NamelessDevelopers/EliminaBot",
            icon_url=self.bot.user.avatar_url,
        )
        embedHelp.set_footer(text="Requested by: " + ctx.message.author.name)
    ...
    async def info(self, ctx: commands.Context) -> None:
        ...
        if botStr == "":
            ...
            embedInfo.add_field(
                name="Following bots are ignored: ", value=botStr, inline=False
            )
        ...
        embedInfo.add_field(
            name="Timer set at: __" + str(DATA[val][0]) + " seconds__",
            value="\u200b",
            inline=True,
        )
